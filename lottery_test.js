...
describe("Lottery Tests", function(){
    ...
    it("Users buy a ticket", async function () {
        const artifactsPath = 'contracts/artifacts/Lottery.json' // Change this for different path
        const metadata = JSON.parse(await remix.call('fileManager', 'getFile', artifactsPath))
        const accounts = await web3.eth.getAccounts()
        let contract = new web3.eth.Contract(metadata.abi)
    ...
    it("Drawn Numbers", async function () {
        const artifactsPath = 'contracts/artifacts/Lottery.json' // Change this for different path
        const metadata = JSON.parse(await remix.call('fileManager', 'getFile', artifactsPath))
        const accounts = await web3.eth.getAccounts()
        let contract = new web3.eth.Contract(metadata.abi)
       ...
       // console.log("This round will end at block: " + duration);
        ...
        const checkResult = await try_contract.methods.drawNumbers().send({
            from: accounts[0],
            gas: 15000000000,
            gasPrice: '30000000000'
        })

        assert.equal(checkResult,true, "drawNumber fails!")
